class Manage::EventsController < Manage::RootController
  ...
  def index
    @manage_events = Manage::Event.all
  ...
  def show
    @manage_event = Manage::Event.find(params[:id])
  ...
  def create
    @manage_event = Manage::Event.new(params[:manage_event])

    respond_to do |format|
      if @manage_event.save
        format.html { redirect_to @manage_event, notice: 'Event was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @manage_event.update_attributes(params[:manage_event])
        format.html { redirect_to @manage_event, notice: 'Event was successfully updated.' }
        format.json { head :ok }
      else
